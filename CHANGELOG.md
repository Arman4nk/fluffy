...
Rayka v1.22.0 brings a new design for spaces, replaces the bottom navigation bar with filter
...
Rayka also now uses the new authenticated media endpoints if the server supports Matrix v1.11
or
...
Rayka v1.21.0 introduces the new search feature which also includes a gallery and files list
...
Rayka v1.19.0 features an improved design for message bubbles and a lot of fixes under the
hood.
...
- fix: Rayka should assume m.change_password capabilitiy is supported if not present per spec (
  krille-chan)
...
Rayka v1.17.0 refreshes the overall user experience, changes some design and fixes a lot of
...
Minor bugfix release. Please note that because of a Flutter update Rayka is no longer
...
Release with a lot of bugfixes and refactorings under the hood. Rayka now uses go_router
...
Rayka 1.7.0 features a new way to work with spaces via a bottom navigation bar. A lot of work
...
Rayka 1.6.0 features a lot of bug fixes and improvements. The code base has been
...
Rayka 1.3.0 makes it possible to report offensive users to server admins (not only messages).
It fixes
...
Rayka 1.3.0 is the first release with full null safe dart code. While this is a huge change
under the
...
Rayka 1.2.0 brings a new stories feature, a lot of bug fixes and improved
voice messages.
...
- fix: Temporary disable sign-up for matrix.org (Currently gives "500: Internal Server Error" while
  Rayka **should** send the same requests like Element)
...
Rayka 0.32.0 targets improved stability and a new onboarding flow where single sign on is now
